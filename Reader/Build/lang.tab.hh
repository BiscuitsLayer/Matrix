  ...
  private:
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_command = 12,                          // command
        S_edge = 13,                             // edge
        S_values = 14,                           // values
        S_resistance = 15,                       // resistance
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, RV&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const RV& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_RESISTANCE (location_type l)
      {
        return symbol_type (token::RESISTANCE, std::move (l));
      }
#else
      static
      symbol_type
      make_RESISTANCE (const location_type& l)
      {
        return symbol_type (token::RESISTANCE, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_VOLTAGE (location_type l)
      {
        return symbol_type (token::VOLTAGE, std::move (l));
      }
#else
      static
      symbol_type
      make_VOLTAGE (const location_type& l)
      {
        return symbol_type (token::VOLTAGE, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_DOUBLEDASH (location_type l)
      {
        return symbol_type (token::DOUBLEDASH, std::move (l));
      }
#else
      static
      symbol_type
      make_DOUBLEDASH (const location_type& l)
      {
        return symbol_type (token::DOUBLEDASH, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_UINT (unsigned int v, location_type l)
      {
